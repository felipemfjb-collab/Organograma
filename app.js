...
function renderSubtree(node, depth, parentNode){
  ...
  let childrenEl=null, vDown=null, assistantsEl=null, vAsst=null;
  ...
  if(hasChildren || hasAssistants){
    ...
    btn.addEventListener("click",(e)=>{
      ...
      if(vAsst) vAsst.style.display = val;
  ...
  if(hasAssistants){
    // Espaço vertical entre o card do gestor e os assistentes
    vAsst = document.createElement("div");
    vAsst.className = "v-line";
    vAsst.style.height = "10px";
    wrap.appendChild(vAsst);

    assistantsEl = document.createElement("div");
    assistantsEl.className = "assistants-below";
    assistantsEl.style.cssText = "display:flex;flex-direction:column;align-items:center;width:100%;position:relative;";
    ...
    // Thin vertical connector joining assistantsEl bottom to vDown
    const aConnector = document.createElement("div");
    aConnector.style.cssText = "width:2px;height:8px;background:var(--accent);";
    wrap.appendChild(assistantsEl);
    wrap.appendChild(aConnector);
  }
  ...
  if(hasChildren){
    ...
    vDown.style.cssText = (vDown.style.cssText||"") + ";margin-top:" + (hasAssistants ? "-4px" : "0");
